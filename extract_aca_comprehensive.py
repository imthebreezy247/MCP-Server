#!/usr/bin/env python3
"""
Comprehensive version: Extract ACA email details with multiple format support
"""
...
def extract_client_data(body: str, subject: str, internal_date: str) -> Dict[str, Any]:
    """Extract client data from email body - handles multiple formats"""
    ...
    # FORMAT 1: Standard format with "NAME\nPOLICY\nMonthly Premium:"
    # Handle multiple levels of quote markers (>, >>, >>>, etc.)
    name_match = re.search(r'(?:>{1,10})?\s*([A-Z][A-Z\s]+[A-Z])\s*\n\s*(?:>{1,10})?\s*([A-Z0-9]{8,})\s*\n\s*(?:>{1,10})?\s*Monthly Premium:', body)
    if name_match:
        name = proper_case_name(name_match.group(1).strip())
        policy_number = name_match.group(2).strip()

    # FORMAT 2: Alternative format "Name\nPOLICY TYPE\nCity\nDOB" (like Herschel Bryant)
    if not name and subject:
        # Extract name from subject line
        subject_match = re.search(r'(?:Fwd:|Re:)?\s*([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)\s+ACA', subject, re.IGNORECASE)
        if subject_match:
            name = proper_case_name(subject_match.group(1).strip())

    # If still no name, try to find name from body in alternative format
    if not name:
        # Look for a name pattern at the start of a line
        name_patterns = [
        ...
        for pattern in name_patterns:
            ...
            if match:
                ...
                # Verify it's not a header or signature
                if 'Berman' not in potential_name and 'Shannahan' not in potential_name:
                    name = proper_case_name(potential_name)
                    break

    # Policy number (if not already found)
    if not policy_number:
        policy_match = re.search(r'(?:>{1,10})?\s*([A-Z0-9]{8,})\s*\n\s*(?:>{1,10})?\s*Monthly Premium:', body)
        if policy_match:
    ...
    if dob_match:
        ...
        # Try standalone DOB without age
        dob_match = re.search(r'\b(\d{1,2}/\d{1,2}/\d{4})\b', body)
        if dob_match:
            # Make sure it's not the application date or paid to date
            potential_dob = dob_match.group(1)
            if potential_dob != app_date and potential_dob != paid_to_date:
                dob = potential_dob
    ...
    # Email - prioritize client emails
    email_matches = re.findall(r'([A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,})', body, re.IGNORECASE)
    for em in email_matches:
        em_lower = em.lower()
        if 'danielberman' not in em_lower and 'chris@' not in em_lower and 'ushealth' not in em_lower and 'gabrieldemuro' not in em_lower:
            email = em_lower
            break

    # Address - two line format
    address_match = re.search(r'([0-9]+\s+[A-Z\s]+(?:ST|AVE|RD|DR|BLVD|LN|CT|WAY|PL|WALK)[^\n]*)\s*\n\s*(?:>{1,10}|<)?[^>]*\s*([A-Z\s]+,\s*[A-Z]{2}\s+\d{5})', body, re.IGNORECASE)
    if address_match:
        address = f"{address_match.group(1).strip()}, {address_match.group(2).strip()}"
    else:
        # Try simpler address pattern (street\ncity)
        address_match = re.search(r'(\d+\s+[A-Za-z\s]+)\s*\n\s*([A-Za-z\s]+,\s*[A-Z]{2}\s+\d{5})', body)
        if address_match:
            address = f"{address_match.group(1).strip()}, {address_match.group(2).strip()}"
    ...
    try:
        ...
        for i, msg in enumerate(messages, 1):
            ...
            try:
                ...
                client_info = extract_client_data(body, subject, internal_date)

                # Include emails that have at least a name OR (phone AND email)
                if client_info['Name'] or (client_info['Phone'] and client_info['Email']):
                    client_data.append(client_info)
                    print(f"  Extracted: {client_info.get('Name', 'N/A')} - Policy: {client_info.get('Policy Number', 'N/A')}", file=sys.stderr)
                else:
                    print(f"  Skipped: Insufficient client data in this email", file=sys.stderr)
...
def main():
    """Main function"""
    print("Gmail ACA Email Extractor - Comprehensive Version", file=sys.stderr)

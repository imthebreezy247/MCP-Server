#!/usr/bin/env python3
"""
Extract ACA emails batch 3 (emails 31-45)
...
def main():
    ...
    try:
        # Get all matching emails (we need to get more than 30 to skip to 31-45)
        print("\nSearching for emails...")
        results = service.users().messages().list(
            userId='me',
            q=query,
            maxResults=45  # Get first 45 emails
        ).execute()
        ...
        if total_found < 31:
            print(f"ERROR: Only {total_found} emails found. Need at least 31 to skip first 30.")
            sys.exit(1)

        # Skip first 30, process 31-45 (indices 30-44)
        messages_to_process = messages[30:45]  # This gets emails 31-45
        print(f"Processing emails 31-45 ({len(messages_to_process)} emails)...")

        extracted_data = []

        for idx, msg in enumerate(messages_to_process, start=31):
            print(f"\nProcessing email {idx}/{min(45, total_found)}...")
        ...
        for idx, data in enumerate(extracted_data, start=31):
            print(f"\nEmail {idx}:")
